...
## ENI Swarm Status Endpoints

| Endpoint | Description |
|----------|-------------|
| `GET /health` | Health check |
| `GET /api/builders` | Builder pool status |
| `GET /api/power-levels` | Available power levels |
| `POST /api/submit` | Submit task |
| `GET /api/status/{task_id}` | Task status |
| `GET /api/status/{task_id}/thinking` | Thinking stream |
...
| Parallel tasks | Sequential | **True parallel** |
