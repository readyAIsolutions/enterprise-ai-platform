...
#
# PITFALL FIXED Aug 2026: the original grep token pattern only caught the markdown
# H1 (STATUS_... — IDLE). It never matched the INI cache mirror, so on a genuine
# steady-state idle pass only 4/5 mirrors tokenized as IDLE and the script exited 1
# (false "not fully reconciled"), sending the pass into unnecessary rewrite work.
# Now it tokenizes BOTH formats and counts the per-mirror IDLE state directly.
#
