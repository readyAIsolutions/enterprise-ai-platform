"""ENI module plugin: icm (Legacy Core).
...
_MOD = {"name": "icm", "category": "Legacy Core", "version": "1.0.0",
        "purpose": "ICM (Interpretable Context Methodology) — skill/prompt-engineering layer. Registers the ICM engine as a first-class Platform Kernel module so the rest of the stack can: * scaffold standard numbered", "facades": ["health_check", "initialize", "new_project", "route", "shutdown", "stage_context"]}
...
def _on_transform_tool_result(result, **_):
    try:
        ...
        if tag in blob:
            ...
            lines.append("  Tests: python3 -m pytest modules/icm/tests -q")
