"""ENI module plugin: youtube_transcripts (Legacy Core).
...
_MOD = {"name": "youtube_transcripts", "category": "Legacy Core", "version": "1.0.0",
        "purpose": "YouTube transcript puller with PIA VPN IP rotation. Pulls full-channel or per-video transcripts (yt-dlp + youtube-transcript-api), rotating the exit IP through Private Internet Access so a whole-chan", "facades": ["health_check", "initialize", "last_run", "list_videos", "pull", "shutdown"]}


def _on_transform_tool_result(result, **_):
    try:
        tag = "youtube_transcripts"
        ...
        if tag in blob:
            ...
            lines.append("  Tests: python3 -m pytest modules/youtube_transcripts/tests -q")
