"""ENI module plugin: prompt_guard (Legacy Core).
...
_MOD = {"name": "prompt_guard", "category": "Legacy Core", "version": "1.0.0",
        "purpose": "Enterprise Prompt Guard OS Module — injection / jailbreak / policy guarding. A stdlib-only facade that inspects an inbound prompt and returns a :class:`GuardVerdict` deciding whether it may proceed t", "facades": []}
...
def _on_transform_tool_result(result, **_):
    try:
        ...
        if tag in blob:
            ...
            lines.append("  Tests: python3 -m pytest modules/prompt_guard/tests -q")
