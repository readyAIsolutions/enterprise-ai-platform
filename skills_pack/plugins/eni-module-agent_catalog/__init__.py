"""ENI module plugin: agent_catalog (Legacy Core).
...
_MOD = {"name": "agent_catalog", "category": "Legacy Core", "version": "1.0.0",
        "purpose": "ENI Agent Catalog Module -- unified specialist-agent registry. Fuses two complementary specialist-agent catalogs into one normalized, searchable platform module: * **Codex** subagents (~/Desktop/", "facades": ["facade", "health_check", "initialize", "set_event_bus", "shutdown"]}
...
def _on_transform_tool_result(result, **_):
    try:
        ...
        if tag in blob:
            ...
            lines.append("  Tests: python3 -m pytest modules/agent_catalog/tests -q")
