"""ENI module plugin: agent_os (Legacy Core).
...
_MOD = {"name": "agent_os", "category": "Legacy Core", "version": "1.0.0",
        "purpose": "Enterprise Agent OS Module — a unified AI agent operating system. Replicates Julian Goldie's 'Agent OS' (Hermes + Oracle + Paperclip + Jarvis) as a real, working ENI Enterprise module — but FOSS and", "facades": ["health_check", "initialize", "run", "shutdown", "surface"]}
...
def _on_transform_tool_result(result, **_):
    try:
        ...
        if tag in blob:
            ...
            lines.append("  Tests: python3 -m pytest modules/agent_os/tests -q")
