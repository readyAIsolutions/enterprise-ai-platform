"""ENI module plugin: eval_gate (Legacy Core).
...
_MOD = {"name": "eval_gate", "category": "Legacy Core", "version": "1.0.0",
        "purpose": "ENI Eval Gate OS Module — automated LLM evaluation gates (local & offline). Local, stdlib-only automated evaluation of model output quality using fully deterministic lexical/heuristic metrics (no ext", "facades": []}
...
def _on_transform_tool_result(result, **_):
    try:
        ...
        if tag in blob:
            ...
            lines.append("  Tests: python3 -m pytest modules/eval_gate/tests -q")
