"""ENI module plugin: secret_broker (Legacy Core).
...
_MOD = {"name": "secret_broker", "category": "Legacy Core", "version": "1.0.0",
        "purpose": "Enterprise Secret Broker OS Module — local-first secret handling. Keeps secrets away from remote LLM/API calls via the reversible local-first flow ``detect -> substitute -> call model -> reverse-subs", "facades": []}
...
def _on_transform_tool_result(result, **_):
    try:
        ...
        if tag in blob:
            ...
            lines.append("  Tests: python3 -m pytest modules/secret_broker/tests -q")
