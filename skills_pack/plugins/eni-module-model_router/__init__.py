"""ENI module plugin: model_router (Legacy Core).
...
_MOD = {"name": "model_router", "category": "Legacy Core", "version": "1.0.0",
        "purpose": "ENI Model Router OS Module — enterprise model-routing / fallback gateway. A production-grade, stdlib-only model-routing gateway ripped from the LiteLLM retry/cooldown pattern. It manages a fleet of m", "facades": []}
...
def _on_transform_tool_result(result, **_):
    try:
        ...
        if tag in blob:
            ...
            lines.append("  Tests: python3 -m pytest modules/model_router/tests -q")
