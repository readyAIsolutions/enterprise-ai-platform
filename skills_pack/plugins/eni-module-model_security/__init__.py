"""ENI module plugin: model_security (Legacy Core).
...
_MOD = {"name": "model_security", "category": "Legacy Core", "version": "1.0.0",
        "purpose": "ENI Model Security OS Module. Model-agnostic security pipeline providing infrastructure-level guards that work regardless of whether the underlying model is compromised. Implements OWASP LLM Top 10 m", "facades": []}
...
def _on_transform_tool_result(result, **_):
    try:
        ...
        if tag in blob:
            ...
            lines.append("  Tests: python3 -m pytest modules/model_security/tests -q")
