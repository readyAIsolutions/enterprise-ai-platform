"""ENI module plugin: llmops_trace (Legacy Core).
...
_MOD = {"name": "llmops_trace", "category": "Legacy Core", "version": "1.1.0",
        "purpose": "ENI LLMOps Trace OS Module — local, offline LLM tracing & observability. A langfuse-style, fully-offline tracing/observability subsystem for LLM applications. Mirrors the ``model_security`` / ``eval_", "facades": []}
...
def _on_transform_tool_result(result, **_):
    try:
        ...
        if tag in blob:
            ...
            lines.append("  Tests: python3 -m pytest modules/llmops_trace/tests -q")
