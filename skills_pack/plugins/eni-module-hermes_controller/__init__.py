"""ENI module plugin: hermes_controller (Legacy Core).
...
_MOD = {"name": "hermes_controller", "category": "Legacy Core", "version": "1.0.0",
        "purpose": "Enterprise Hermes Controller OS Module — autonomous controller for Hermes Agent. A stdlib-only facade that wraps the ENI Hermes Controller (prompt expander + router + privacy boundary + reinforcement", "facades": ["facade", "health_check", "initialize", "shutdown"]}


def _on_transform_tool_result(result, **_):
    try:
        tag = "hermes_controller"
        ...
        if tag in blob:
            ...
            lines.append("  Tests: python3 -m pytest modules/hermes_controller/tests -q")
