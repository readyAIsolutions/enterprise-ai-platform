"""ENI module plugin: vuln_scanner (Legacy Core).
...
_MOD = {"name": "vuln_scanner", "category": "Legacy Core", "version": "1.0.0",
        "purpose": "ENI Vuln Scanner OS Module — offline LLM vulnerability scanning (garak-style). Local, stdlib-only, black-box vulnerability scanning of LLM endpoints using deterministic heuristic probes (keywords / r", "facades": []}
...
def _on_transform_tool_result(result, **_):
    try:
        ...
        if tag in blob:
            ...
            lines.append("  Tests: python3 -m pytest modules/vuln_scanner/tests -q")
