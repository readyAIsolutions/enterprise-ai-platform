...
from enterprise.platform_kernel import Module, HealthStatus, module
...
FAKE_DATA_MARKERS_STRONG = [
    "lorem ipsum", "fake data", "dummy data", "for demo only",
    "this is a placeholder", "fizzbuzz", "mock data", "sample data only",
    "no real data", "hardcoded sample", "return fake", "stub for demo",
]
...
NON_SOURCE_SUFFIXES = {
    ".pyc", ".pyo", ".png", ".jpg", ".jpeg", ".gif", ".ico", ".woff", ".woff2",
    ".ttf", ".mp3", ".mp4", ".zip", ".tar", ".gz", ".so", ".dll", ".exe",
    ".pdf", ".lock", ".sqlite", ".db", ".bin", ".npy",
}
SOURCE_SUFFIXES = {".py", ".js", ".ts", ".tsx", ".jsx", ".go", ".rs", ".java",
                   ".rb", ".php", ".c", ".cpp", ".h", ".cs", ".sh", ".sql"}
...
PROJECT_FILE_BLACKLIST = {".git", "__pycache__", "node_modules", ".venv", "venv",
                          ".ruff_cache", ".mypy_cache", ".pytest_cache", "dist",
                          "build", "target", ".tox", "coverage"}
...
def _iter_project_files(root: Path) -> List[Path]:
    ...
    for dirpath, dirnames, filenames in os.walk(root):
        ...
        for fn in filenames:
            p = Path(dirpath) / fn
            # skip hidden + pyc
            if any(seg.startswith(".") and seg not in (".github", ".gitignore")
                   for seg in p.parts):
                continue
            out.append(p)
    return out
...
class UniversalBuildScore:
    ...
    def _probe_builds_clean(self, project: Path) -> SubSignal:
        ...
        if not py_files:
            ...
            return SubSignal("builds_clean", "Builds/parses clean", 1.0, "no python source to compile")
        errs = 0
        for p in py_files:
            try:
        ...
        return SubSignal("builds_clean", "Builds / parses clean", round(score, 4),
                         "; ".join(evidence[:3]))
    ...
    def _probe_error_handling(self, project: Path) -> SubSignal:
        ...
        if try_count == 0:
            return SubSignal("error_handling", "Error handling", 0.5,
                             "no try/except found")
        ratio = min(1.0, except_count / max(1, try_count))
        # if every try has >=1 handler -> strong
        score = min(1.0, 0.4 + 0.6 * ratio)
        return SubSignal("error_handling", "Error handling", round(score, 4),
    ...
    def _probe_no_crashes(self, project: Path) -> SubSignal:
        ...
        return SubSignal("no_crashes", "Defensive / no crashes", round(score, 4),
    ...
    def _probe_no_hardcoded_secrets(self, project: Path) -> SubSignal:
        ...
        for p in _iter_project_files(project):
            if p.suffix not in (".py", ".js", ".ts", ".env", ".json", ".yaml", ".yml", ".toml", ".sh"):
                continue
            low_name = p.name.lower()
            # Skip test/fixture/spec files — they legitimately contain fake
            # credentials, and skip regex/pattern definition files.
            if "test" in low_name or "fixture" in low_name or "spec" in low_name \
               or "pattern" in low_name or "regex" in low_name or "example" in low_name:
                continue
            ...
            for line in text.splitlines():
                ...
                for name, pattern in SECRET_PATTERNS:
                    ...
                    if ("PRIVATE KEY" in line and ("secret" in line.lower()
        ...
        if hits:
            ...
            return SubSignal("no_hardcoded_secrets", "No hardcoded secrets", 0.0,
                             f"FOUND: {', '.join(uniq)}")
        return SubSignal("no_hardcoded_secrets", "No hardcoded secrets", 1.0,
    ...
    def _probe_deps_pinned(self, project: Path) -> SubSignal:
        ...
        for p in _iter_project_files(project):
            ...
            if name in ("requirements.txt", "requirements.in"):
                text = p.read_text(encoding="utf-8", errors="ignore")
                pinned = sum(1 for ln in text.splitlines()
                ...
                score = max(0.2, ratio)
                return SubSignal("deps_pinned", "Dependencies pinned", round(score, 4),
                                 "; ".join(evidence))
            elif name in ("poetry.lock", "uv.lock", "Pipfile.lock", "pnpm-lock.yaml",
                          "package-lock.json", "yarn.lock", "go.sum", "Cargo.lock"):
                return SubSignal("deps_pinned", "Dependencies pinned", 1.0,
                                 f"lockfile present: {p.name}")
        return SubSignal("deps_pinned", "Dependencies pinned", 0.4,
                         "no lockfile/pinning found")

    def _probe_gitignore_clean(self, project: Path) -> SubSignal:
        gi = project / ".gitignore"
        if not gi.exists():
            return SubSignal("gitignore_clean", ".gitignore & repo hygiene", 0.3,
                             "no .gitignore")
        ...
        if any(k in text for k in ("*.log", "dist", "build")):
            score += 0.10
        return SubSignal("gitignore_clean", ".gitignore & repo hygiene",
                         round(min(1.0, score), 4), ".gitignore present")
    ...
    def _probe_type_hints(self, project: Path) -> SubSignal:
        ...
        for p in py_files:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.FunctionDef):
                    ...
                    has_args = bool(node.args.args and
                                    all(a.annotation for a in node.args.args if a.arg not in ("self", "cls")))
                    if has_ret:
                        annotated += 1
                    if has_args:
                        with_hints += 1
        ...
        return SubSignal("type_hints", "Type hints", round(min(1.0, ratio * 2), 4),
    ...
    def _probe_docstrings(self, project: Path) -> SubSignal:
        ...
        return SubSignal("docstrings", "Docstrings", round(min(1.0, ratio * 1.5), 4),
    ...
    def _probe_structure(self, project: Path) -> SubSignal:
        ...
        has_main = any(p.name in ("main.py", "app.py", "__main__.py", "cli.py")
                       for p in py_files)
        score = 0.4
        if has_packages >= 1:
            score += 0.25
        ...
        if has_main:
            score += 0.20
        return SubSignal("structure", "Project structure", round(min(1.0, score), 4),
    ...
    def _probe_tests_pass(self, project: Path, run_tests: bool = True) -> SubSignal:
        test_files = [p for p in _iter_project_files(project)
                      if p.suffix == ".py" and ("test" in p.name.lower()
                                                or "tests" in str(p.parent).lower())]
        if not test_files:
            return SubSignal("tests_pass", "Tests pass", 0.0, "no tests to run")
        if not run_tests:
            return SubSignal("tests_pass", "Tests pass", 0.5,
                             "test run disabled by config")
        # Try actually running pytest on the project
        try:
            cmd = [self._python_bin, "-m", "pytest", "-p", "no:cacheprovider",
                   "-q", "--no-header", "-x", str(project)]
            proc = subprocess.run(
            ...
            if total == 0:
                return SubSignal("tests_pass", "Tests pass", 0.3,
                                 "pytest collected 0 tests: " + out[-80:])
            if failed or errors:
                return SubSignal("tests_pass", "Tests pass",
                                 round(max(0.0, passed / total), 4),
                                 f"{passed} passed, {failed} failed, {errors} errors")
            return SubSignal("tests_pass", "Tests pass", 1.0,
                             f"{passed} passed, 0 failed")
    ...
    def _probe_test_to_source_ratio(self, project: Path) -> SubSignal:
        py_files = [p for p in _iter_project_files(project) if p.suffix == ".py"]
        src_lines = sum(_count_lines(p) for p in py_files
                        if "test" not in p.name.lower())
        test_lines = sum(_count_lines(p) for p in py_files
        ...
        return SubSignal("test_to_source_ratio", "Test : source ratio",
                         round(score, 4),
    ...
    def _probe_coverage_heuristic(self, project: Path,
        ...
        if cov_files:
            return SubSignal("coverage_heuristic", "Coverage", round(min(1.0, base * 1.2 + 0.1), 4),
                             "coverage artifacts present")
        return SubSignal("coverage_heuristic", "Coverage", round(base * 0.8, 4),
    ...
    def _probe_ci_config(self, project: Path) -> SubSignal:
        ...
        for p in _iter_project_files(project):
            ...
            if p.name in (".github", "azure-pipelines.yml", "bitbucket-pipelines.yml"):
                hits.append(p.name)
        if hits:
            return SubSignal("ci_config", "CI/CD config", 1.0, f"CI found: {', '.join(sorted(set(hits))[:4])}")
        return SubSignal("ci_config", "CI/CD config", 0.0, "no CI config found")

    def _probe_packaging(self, project: Path) -> SubSignal:
        hits = []
        for p in _iter_project_files(project):
            if p.name in ("pyproject.toml", "setup.py", "setup.cfg", "setup.sh",
                          "Makefile", "package.json", "Cargo.toml", "go.mod"):
                hits.append(p.name)
        score = min(1.0, len(hits) * 0.35)
        return SubSignal("packaging", "Packaging / build tooling", round(score, 4),
    ...
    def _probe_versioned(self, project: Path) -> SubSignal:
        ...
        for p in _iter_project_files(project):
            if p.name in ("pyproject.toml", "setup.py", "package.json", "Cargo.toml"):
                text = p.read_text(encoding="utf-8", errors="ignore")
                if re.search(r"version\s*=\s*['\"]\d+\.\d+", text) or \
                   re.search(r'"version"\s*:\s*"\d+\.\d+', text):
                    version_refs += 1
        is_git = (project / ".git").exists()
        score = min(1.0, 0.5 * version_refs + (0.5 if is_git else 0))
        return SubSignal("versioned", "Versioned / git", round(score, 4),
    ...
    def _probe_docs(self, project: Path) -> SubSignal:
        ...
        for p in _iter_project_files(project):
            low = str(p).lower()
            if any(k in low for k in ("/docs/", "/doc/", "docs.md", "user_guide",
                                      "manual", "api.md", "getting_started")):
                docs_found.append(p.name)
        if docs_found:
            return SubSignal("docs", "Documentation", 1.0,
                             f"docs: {', '.join(sorted(set(docs_found))[:5])}")
        return SubSignal("docs", "Documentation", 0.2, "no docs directory")

    def _probe_no_fake_data(self, project: Path) -> SubSignal:
        py_files = [p for p in _iter_project_files(project)
                    if p.suffix == ".py" and "test" not in p.name.lower()
                    and "fixture" not in p.name.lower() and "example" not in p.name.lower()
        ...
        if hits:
            ...
            return SubSignal("no_fake_data", "No fake/placeholder data", 0.0,
                             f"FOUND: {', '.join(uniq)}")
        return SubSignal("no_fake_data", "No fake/placeholder data", 1.0,
    ...
    def _probe_real_persistence(self, project: Path) -> SubSignal:
        ...
        for p in py_files:
            for line in p.read_text(encoding="utf-8", errors="ignore").splitlines():
                s = line.strip()
                if s.startswith(("import sqlite3", "import redis", "from sqlalchemy",
                                 "import psycopg2", "import pymongo", "import tinydb",
                                 "from django.db", "import duckdb", "import aiosqlite",
                                 "import peewee", "import pickledb", "import pickle"
                                 )):
                    imports.append(s)
        if imports:
            return SubSignal("real_persistence", "Real persistence", 1.0,
        ...
        for p in py_files:
            if re.search(r"open\([^)]*['\"]w['\"]", p.read_text(encoding="utf-8", errors="ignore")):
                return SubSignal("real_persistence", "Real persistence", 0.6,
                                 "file-write persistence found")
        return SubSignal("real_persistence", "Real persistence", 0.2,
    ...
    def _dimension_probes(self, dimension: Dimension, project: Path,
        ...
        # SELLABILITY
        return [self._probe_readme(project),
    ...
    def _hard_gates(self, project: Path, tests_pass: SubSignal,
                    secrets: SubSignal, fake_data: SubSignal,
                    persistence: SubSignal, builds_clean: SubSignal) -> List[HardGate]:
        gates: List[HardGate] = []
        gates.append(HardGate("builds", builds_clean.score >= 0.5,
                              builds_clean.evidence))
        if tests_pass.score < 0.99 and tests_pass.evidence and "passed" in tests_pass.evidence \
           and tests_pass.score == 0.0:
            gates.append(HardGate("tests_pass", False, tests_pass.evidence))
        else:
            gates.append(HardGate("tests_pass", tests_pass.score >= 0.5,
                                  tests_pass.evidence))
        gates.append(HardGate("no_hardcoded_secrets", secrets.score >= 0.99,
                              secrets.evidence))
        gates.append(HardGate("no_fake_data", fake_data.score >= 0.99, fake_data.evidence))
        gates.append(HardGate("real_persistence", persistence.score >= 0.3,
    ...
    def score(self, project_path: Any, run_tests: bool = True,
        ...
        # Hard gates
        gates = self._hard_gates(project, tests_pass, secrets, fake_data,
                                 persistence, builds_clean)
        ...
        return {
            ...
            "hard_gates": [{"name": g.name, "passed": g.passed, "detail": g.detail}
                           for g in gates],
...
@module(name="universal_score", version="1.0.0", config_defaults={
...
class UniversalScoreModule(Module):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.engine: Optional[UniversalBuildScore] = None
...
def create_universal_score_module(config: Optional[Dict[str, Any]] = None) -> UniversalScoreModule:
