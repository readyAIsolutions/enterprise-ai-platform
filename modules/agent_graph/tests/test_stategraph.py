...
def test_conditional_edge_follows_router():
    ...
    g.add_node("right", lambda s: {"branch": "R"})
...
def test_conditional_edge_routes_to_other_branch():
    ...
    g.add_node("left", lambda s: {"branch": "L"})
    g.add_node("right", lambda s: {"branch": "R"})
    g.add_conditional_edge("decide", lambda s: "right")
    g.add_edge("left", END)
    g.add_edge("right", END)
    run = g.run({})
...
def test_resume_completed_run_is_noop_and_skips_nodes():
    ...
    g.add_node("b", lambda s: runs.__setitem__("b", runs["b"] + 1) or {})
    g.add_edge("a", "b")
    g.add_edge("b", END)
...
def test_resume_across_store_reopen(tmp_path):
    ...
    g1.add_node("a", lambda s: {"a": 1})
    g1.add_node("b", node_b)
    g1.add_node("c", lambda s: {"c": 1})
    g1.add_edge("a", "b")
    g1.add_edge("b", "c")
    g1.add_edge("c", END)
    ...
    g2.add_node("a", lambda s: {"a": 1})
    g2.add_node("b", node_b)
    g2.add_node("c", lambda s: {"c": 1})
    g2.add_edge("a", "b")
    g2.add_edge("b", "c")
    g2.add_edge("c", END)
    run = g2.resume("dur-1")
...
def test_supervisor_fans_out_to_subgraph():
    ...
    parent.add_node("pre", lambda s: {"val": 21})
