...
def _seed_good(store: SkillFeedbackStore, name: str = "alpha", n: int = 20, day: int = 1) -> None:
    for _ in range(n):
        store.record_outcome(
            skill=name, result=RESULT_SUCCESS, feedback=10,
            latency=0.0, iterations=1, timestamp=_ts(day),
        )
...
def _seed_bad(store: SkillFeedbackStore, name: str = "gamma", n: int = 12, day: int = 1) -> None:
    for _ in range(n):
        store.record_outcome(
            skill=name, result=RESULT_FAIL, feedback=0,
            latency=8.0, iterations=8, timestamp=_ts(day),
        )
...
class TestSkillFeedbackStore:
    def test_record_and_read_outcome(self, tmp_path):
        ...
        row_id = store.record_outcome(
            skill="alpha", result="success", score=0.9,
            latency=1.2, iterations=3, feedback=7, timestamp=_NEWER,
        )
    ...
    def test_count_and_all_skills(self, tmp_path):
        store = _make(tmp_path)
...
class TestSkillScore:
    def test_empty_outcomes_zero_weight(self, tmp_path):
        store = _make(tmp_path)
        score = compute_evolution_score("ghost", store, now=_NOW_EPOCH)
        assert score["n"] == 0
        assert score["weight"] == 0.0

    def test_all_success_high_score(self, tmp_path):
        store = _make(tmp_path)
        _seed_good(store)
    ...
    def test_success_rate_orders_scores(self, tmp_path):
        store = _make(tmp_path)
    ...
    def test_feedback_raises_score(self, tmp_path):
        store = _make(tmp_path)
        for _ in range(20):
            store.record_outcome("liked", RESULT_SUCCESS, feedback=10,
                                 latency=0, iterations=1, timestamp=_ts(1))
            store.record_outcome("flat", RESULT_SUCCESS, feedback=0,
                                 latency=0, iterations=1, timestamp=_ts(1))
    ...
    def test_efficiency_raises_score(self, tmp_path):
        store = _make(tmp_path)
        for _ in range(20):
            store.record_outcome("fast", RESULT_SUCCESS, feedback=5,
                                 latency=0.1, iterations=1, timestamp=_ts(1))
            store.record_outcome("slow", RESULT_SUCCESS, feedback=5,
                                 latency=20.0, iterations=20, timestamp=_ts(1))
    ...
    def test_recency_boosts_newer_skill(self, tmp_path):
        store = _make(tmp_path)
        for _ in range(20):
            store.record_outcome("fresh", RESULT_SUCCESS, feedback=5,
                                 latency=1, iterations=1, timestamp=_ts(3))
            store.record_outcome("stale", RESULT_SUCCESS, feedback=5,
                                 latency=1, iterations=1, timestamp=_ts(1))
    ...
    def test_volume_weighted_stability_no_overweight(self, tmp_path):
        """A single lucky 100% run must NOT rival a well-measured 100% skill."""
        store = _make(tmp_path)
        store.record_outcome("lucky", RESULT_SUCCESS, feedback=10,
                             latency=0, iterations=1, timestamp=_ts(1))
...
class TestEvolutionEngine:
    def test_record_outcome_delegates_and_returns_row(self, tmp_path):
        store = _make(tmp_path)
    ...
    def test_update_score_computes_weight(self, tmp_path):
        store = _make(tmp_path)
        _seed_good(store)
        engine = EvolutionEngine(store)
    ...
    def test_rank_skills_orders_descending(self, tmp_path):
        store = _make(tmp_path)
        ...
        for i in range(15):
            store.record_outcome(
                ...
                feedback=5, latency=1, iterations=1, timestamp=_ts(1),
            )
        engine = EvolutionEngine(store)
        ...
        top, bottom = engine.identify_top(1, now=_NOW_EPOCH)[0], engine.identify_bottom(1, now=_NOW_EPOCH)[0]
        assert top == ranked[0]
        assert bottom == ranked[-1]

    def test_promote_below_threshold_marks(self, tmp_path):
        store = _make(tmp_path)
    ...
    def test_promote_above_threshold_not_marked(self, tmp_path):
        store = _make(tmp_path)
    ...
    def test_refresh_loop_scores_scores_and_promotes(self, tmp_path):
