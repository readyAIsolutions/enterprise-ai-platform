...
class FeatureFlag:
    ...
    def applies_to(self, context: Optional[Dict[str, Any]]) -> bool:
        """True when targeting rules (if any) all match the context."""
        ctx = context or {}
        return all(rule.matches(ctx) for rule in self.targeting_rules)

    def variant_for(self, context: Optional[Dict[str, Any]]) -> Optional[str]:
...
class FlagEngine:
    ...
    def set_rollout(self, name: str, percent: float) -> bool:
        ...
        self.audit.log(
            "flag.set_rollout", name=name, details={"rollout_percent": percent}
        )
        return True
    ...
    def is_enabled(self, name: str, context: Optional[Dict[str, Any]] = None) -> bool:
        ...
        self.audit.log(
            "flag.evaluate", name=name, decision=decision,
            details={"rollout_percent": flag.rollout_percent, "context_key": context_key},
        )
        return decision

    def get_variant(self, name: str, context: Optional[Dict[str, Any]] = None) -> Optional[str]:
        ...
        self.audit.log(
            "flag.variant", name=name,
            decision=variant is not None, details={"variant": variant},
        )
        return variant
...
class Canary:
    ...
    def send_traffic(self) -> Dict[str, Any]:
        ...
        if self.state is CanaryState.ROLLED_BACK:
            raise RuntimeError(f"canary {self.name} is rolled back; re-issue a new canary")
    ...
    def promote(self, health_check: Optional[Callable[[], bool]] = None) -> Dict[str, Any]:
        ...
        if self.state is CanaryState.NOT_STARTED:
            self.send_traffic()
        if self.state is CanaryState.ROLLED_BACK:
            raise RuntimeError(f"canary {self.name} is rolled back")
        ...
        if healthy is False:
            ...
            self.audit.log(
                "canary.promote_blocked", name=self.name,
                decision=False, details=result,
            )
            return result
        ...
        result = self._snapshot()
        self.audit.log(
            "canary.promote", name=self.name,
            decision=True, details=result,
        )
        return result
    ...
    def rollback(self) -> Dict[str, Any]:
        ...
        result = self._snapshot()
        self.audit.log(
            "canary.rollback", name=self.name,
            decision=True, details=result,
        )
        return result
...
class ReleaseGate:
    ...
    def evaluate(
        ...
        checks.append(
            {"check": "flag_exists", "ok": flag is not None,
        ...
        if flag is not None:
            checks.append(
                {"check": "flag_enabled", "ok": flag.enabled,
        ...
        checks.append(
            {"check": "rollout_within_policy", "ok": rollout_ok,
             "reason": (
                 f"rollout={flag.rollout_percent}<=max={self.max_rollout_percent}"
                 if rollout_ok else "rollout_exceeds_policy"
             )}
        )
        ...
        audit.log(
            "gate.evaluate", name=self.name,
            decision=all_ok, details=result,
        )
        return result
...
class AuditLog:
    ...
    def find(self, action: Optional[str] = None, name: Optional[str] = None) -> List[AuditEntry]:
        out = []
