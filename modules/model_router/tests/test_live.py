...
def _timeout(base_url="http://x.test/v1", model="m", api_key=None, timeout=15.0):
...
def _route_by_url(responses):
    ...
    def _fn(base_url, model="m", api_key=None, timeout=15.0):
        if base_url in responses:
            return responses[base_url](base_url=base_url, model=model, api_key=api_key, timeout=timeout)
        return (200, {})
    return _fn
...
def test_live_probe_reachable_with_injected_response():
...
def test_live_probe_rate_limited_unreachable():
...
def test_live_probe_5xx_unreachable():
    res = LiveProbe().run("http://svc.test/v1", model="m", request_fn=_svc)
    assert res.reachable is False
    assert res.status_code == 503


def test_live_probe_timeout_unreachable():
...
def test_live_probe_result_dict_keys_present():
    res = LiveProbe().run("http://ok.test/v1", model="m", request_fn=_ok)
    d = res.as_dict()
...
def test_failover_picks_healthy_after_429_cooldown():
...
def test_failover_picks_healthy_after_5xx():
    deployments = _mk_deployments()
    smoke = LiveFailoverSmoke(
        deployments,
...
def test_failover_picks_healthy_after_timeout():
    deployments = _mk_deployments()
    smoke = LiveFailoverSmoke(
        deployments,
...
def test_failover_reports_reachable_list():
    deployments = _mk_deployments()
    smoke = LiveFailoverSmoke(
        deployments,
        request_fn=_route_by_url({"http://probe-a.test/v1": _rl}),
...
def test_failover_chain_chooses_non_cooldowned_deployment():
...
def test_failover_smoke_accepts_dict_deployments():
    deployments = [
...
def test_adapter_diagnose_classifies_last_outcome():
    adapter = HTTPAdapter()
...
def test_adapter_diagnose_with_explicit_error():
    adapter = HTTPAdapter()
...
def test_ping_local_free_router_returns_tuple():
    ok, detail = ping_local_free_router(host="127.0.0.1", port=9, timeout=0.2)
    assert isinstance(ok, bool)
    assert isinstance(detail, str)


def test_ping_local_free_router_unreachable(monkeypatch):
    def _refuse(*a, **k):
...
@pytest.mark.skipif(not _local_free_router_up(), reason="local free-router not up at 127.0.0.1:8920")
def test_ping_local_free_router_live_when_up():
