...
class PositionAddressedMemoryModule(Module):
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            if not root:
                # Default to a managed data dir (computed from this module's
                # location) so the module initializes out-of-the-box instead of
                # failing when no explicit 'root' is configured.
                root = str(
                    Path(__file__).resolve().parent / "data" / "position_addressed_memory"
                )
