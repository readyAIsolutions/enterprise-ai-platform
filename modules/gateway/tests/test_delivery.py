...
class TestDeliveryPolicy:
    def test_delay_expands_exponentially(self):
        policy = DeliveryPolicy(
            max_retries=3, base_backoff_seconds=0.1, backoff_factor=2.0
        )
    ...
    def test_delay_respects_cap(self):
        policy = DeliveryPolicy(
            max_retries=5, base_backoff_seconds=4.0, backoff_factor=2.0,
...
class TestSendWithRetry:
    ...
    def test_retries_transient_then_succeeds(self):
        ...
        receipt = send_with_retry(
            ...
            "msg",
            policy=DeliveryPolicy(
                max_retries=4, base_backoff_seconds=0.01, backoff_factor=2.0
            ),
            sleep_fn=sleeper,
        )
        assert receipt.passed is True
    ...
    def test_exhausts_retries_and_fails(self):
        ...
        policy = DeliveryPolicy(
            max_retries=2, base_backoff_seconds=0.01, backoff_factor=2.0
        )
    ...
    def test_does_not_retry_permanent_config_failure(self):
        sleeper = RecordingSleeper()
        ch = FlakyChannel(name="disabled", enabled=False)
        receipt = send_with_retry(
            ch, "x", policy=DeliveryPolicy(max_retries=5), sleep_fn=sleeper
        )
        assert receipt.passed is False
        assert receipt.attempts == 1
        assert sleeper.delays == []

    def test_receipt_fields_recorded(self):
        sleeper = RecordingSleeper()
    ...
    def test_backoff_timing_matches_exponential_sequence(self):
        sleeper = RecordingSleeper()
    ...
    def test_gateway_send_with_retry_integration(self):
        sleeper = RecordingSleeper()
        gateway = Gateway()
...
class TestGatewayRouterHandoff:
    def test_route_success_no_handoff(self):
        sleeper = RecordingSleeper()
    ...
    def test_route_unknown_channel_returns_failed(self):
        router = GatewayRouter()
        receipt = router.route("nope", "x")
        assert receipt.passed is False
        assert receipt.status == DeliveryStatus.FAILED

    def test_handoff_to_fallback_after_primary_failure(self):
        sleeper = RecordingSleeper()
        router = GatewayRouter(
    ...
    def test_no_handoff_when_no_fallback_registered(self):
        sleeper = RecordingSleeper()
        router = GatewayRouter()
        primary = FlakyChannel(name="primary", enabled=False)
        router.registry.register("primary", primary)
    ...
    def test_gateway_router_shares_gateway_registry(self):
        gateway = Gateway()
...
class TestOutbox:
    ...
    def test_drain_counts_failed_and_retried(self):
        sleeper = RecordingSleeper()
        gateway = Gateway()
    ...
    def test_drain_with_no_router_marks_failed(self):
        outbox = Outbox()
        outbox.enqueue("x", "m")
