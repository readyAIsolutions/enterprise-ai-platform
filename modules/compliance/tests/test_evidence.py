...
class TestControlEvidence:
    def test_hash_is_computed_and_stable(self) -> None:
        ev = ControlEvidence(
            "LLM01", "owasp", "implemented", source="scanner-a", detail="verified at t0"
        )
...
class TestIntegrity:
    ...
    def test_integrity_detects_content_tamper(self, register) -> None:
        ...
        register._conn.execute("UPDATE evidence SET detail = 'forged' WHERE control_id = 'LLM01'")
...
class TestFacadesAndLifecycle:
    ...
    def test_assess_per_framework_facade(self) -> None:
    ...
    def test_in_memory_is_ephemeral(self) -> None:
        r = EvidenceRegister()
        r.add(ControlEvidence("LLM01", "owasp", "implemented", source="s"))
        assert r.count() == 1
        r.close()

    def test_module_and_factory_expose_evidence(self) -> None:
        mod = create_compliance_module({"framework": None})
        assert isinstance(mod, ComplianceModule)
        mod.add_evidence(ControlEvidence("LLM01", "owasp", "implemented", source="scan"))
