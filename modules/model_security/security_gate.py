...
class SecurityGate:
    ...
    def execute(
        ...
        with self._lock:
            ...
            result = self.pipeline.execute(prompt, model_fn, model_name, model_type)
            self.stats.total += 1
            if not result.passed:
                self.stats.blocked_input += (
                    1
                    if result.routing_decision.startswith("gate_block")
                    else self.stats.blocked_input
                )
                self.stats.by_action[
                    result.stage_results.get("output_validation", None).action.value
                    if result.stage_results.get("output_validation")
                    else "?"
    ...
    def wrap(
        self, model_fn: Callable[[str], str], model_name: str = "model", model_type: str = "cloud"
    ) -> Callable[[str], PipelineResult]:
